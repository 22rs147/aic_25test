-- aic_2023a_schema.sql
-- phpMyAdmin SQL Dump
-- version 5.2.1
-- https://www.phpmyadmin.net/
--
-- ホスト: 127.0.0.1
-- 生成日時: 2025-07-05 10:04:55
...
CREATE TABLE `rsv_member` (
  ...
  CONSTRAINT `fk_rsv_member_reserve`
    FOREIGN KEY (`reserve_id`)
    REFERENCES `tb_reserve` (`id`)
    ON DELETE CASCADE ON UPDATE CASCADE,

  CONSTRAINT `fk_rsv_member_member`
...
CREATE TABLE `rsv_sample` (
  ...
  CONSTRAINT `fk_rsv_sample_reserve`
    FOREIGN KEY (`reserve_id`)
    REFERENCES `tb_reserve` (`id`)
    ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
CREATE TABLE `tb_reserve` (
  ...
  CONSTRAINT `fk_reserve_instrument`
    FOREIGN KEY (`instrument_id`)
    REFERENCES `tb_instrument` (`id`)
    ON DELETE RESTRICT ON UPDATE CASCADE,

  CONSTRAINT `fk_reserve_apply_member`
    FOREIGN KEY (`apply_mid`)
    REFERENCES `tb_member` (`uid`)
    ON DELETE RESTRICT ON UPDATE CASCADE,

  CONSTRAINT `fk_reserve_master_member`
    FOREIGN KEY (`master_mid`)
    REFERENCES `tb_member` (`uid`)
    ON DELETE RESTRICT ON UPDATE CASCADE,
...
--
-- テーブルのインデックス `rsv_member`
--
ALTER TABLE `rsv_member`
  ADD PRIMARY KEY (`id`);

--
-- テーブルのインデックス `rsv_sample`
--
ALTER TABLE `rsv_sample`
  ADD PRIMARY KEY (`id`);

--
-- テーブルのインデックス `tb_instrument`
--
ALTER TABLE `tb_instrument`
  ADD PRIMARY KEY (`id`);

--
-- テーブルのインデックス `tb_member`
--
...
--
-- テーブルの AUTO_INCREMENT `rsv_member`
--
ALTER TABLE `rsv_member`
  MODIFY `id` bigint(20) UNSIGNED NOT NULL AUTO_INCREMENT COMMENT '通し番号（自動採番, 内部参照用）', AUTO_INCREMENT=3;

--
-- テーブルの AUTO_INCREMENT `rsv_sample`
--
ALTER TABLE `rsv_sample`
  MODIFY `id` bigint(20) UNSIGNED NOT NULL AUTO_INCREMENT COMMENT '通し番号（自動採番, 内部参照用）';

--
-- テーブルの AUTO_INCREMENT `tb_instrument`
--
ALTER TABLE `tb_instrument`
  MODIFY `id` bigint(20) UNSIGNED NOT NULL AUTO_INCREMENT COMMENT '通し番号(自動採番, 内部参照用)', AUTO_INCREMENT=47;

--
-- テーブルの AUTO_INCREMENT `tb_member`
--
ALTER TABLE `tb_member`
  MODIFY `id` bigint(20) UNSIGNED NOT NULL AUTO_INCREMENT COMMENT '通し番号（自動採番, 内部参照用）', AUTO_INCREMENT=125;

--
-- テーブルの AUTO_INCREMENT `tb_reserve`
--
ALTER TABLE `tb_reserve`
  MODIFY `id` bigint(20) UNSIGNED NOT NULL AUTO_INCREMENT COMMENT '通し番号（自動採番, 内部参照用）', AUTO_INCREMENT=18;

--
-- テーブルの AUTO_INCREMENT `tb_room`
--
ALTER TABLE `tb_room`
  MODIFY `id` bigint(20) UNSIGNED NOT NULL AUTO_INCREMENT COMMENT '通し番号（自動採番, 内部参照用）', AUTO_INCREMENT=16;

--
-- テーブルの AUTO_INCREMENT `tb_staff`
--
ALTER TABLE `tb_staff`
  MODIFY `id` bigint(20) UNSIGNED NOT NULL AUTO_INCREMENT COMMENT '通し番号（自動採番, 内部参照用）', AUTO_INCREMENT=21;
COMMIT;